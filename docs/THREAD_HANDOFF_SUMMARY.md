...
生成时间：2026-05-18（2026-07-16 更新至 0.5.1 后台启动稳定性修复）
...
### 当前主线状态（2026-07-16 / 0.5.1 build 33）
...
- 发布脚本 `MeetingBotMenuBarApp/build_release_app.sh` 当前版本为 `0.5.1`、构建号 `33`；Xcode、升级脚本与安装盘脚本版本已同步。
...
- `dist/会议纪要助手.app`
- 默认安装盘路径：`dist/会议纪要助手 0.5.1 安装盘.dmg`
- 默认线程交接导出路径：`dist/线程交接汇总 0.5.1.md`
- 发布元数据路径：`dist/release-manifest-0.5.1.json`

## 已执行验证

2026-07-16 / 0.5.1 主线执行以下检查：
...
- Python 编译检查通过。
- 菜单栏 App 已重新构建，`Info.plist` 版本为 `0.5.1`，构建号为 `33`。
...
- `scripts/build_setup_package.sh` 默认版本号已同步到 `0.5.1`，并重新生成 DMG 与发布元数据。
