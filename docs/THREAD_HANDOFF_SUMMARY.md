...
生成时间：2026-05-18（2026-07-31 更新至 0.5.2 安全与安装边界修复）
...
### 当前主线状态（2026-07-31 / 0.5.2 build 34）
...
- 发布脚本 `MeetingBotMenuBarApp/build_release_app.sh` 当前版本为 `0.5.2`、构建号 `34`；Xcode、升级脚本与安装盘脚本版本已同步。
...
- `dist/会议纪要助手.app`
- 默认安装盘路径：`dist/会议纪要助手 0.5.2 安装盘.dmg`
- 默认线程交接导出路径：`dist/线程交接汇总 0.5.2.md`
- 发布元数据路径：`dist/release-manifest-0.5.2.json`

## 已执行验证

2026-07-31 / 0.5.2 候选版本执行以下检查：
...
- Python 编译检查通过。
- 菜单栏 App 已重新构建，`Info.plist` 版本为 `0.5.2`，构建号为 `34`。
...
- `scripts/build_setup_package.sh` 默认版本号已同步到 `0.5.2`；当前已验证开发版 App 构建与签名完整性，正式 DMG 仍需真实发布签名和 Apple 公证凭据。
