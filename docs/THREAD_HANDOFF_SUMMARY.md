...
生成时间：2026-05-18（2026-05-20 追加 0.2.14 修订轮次；2026-06-16 追加 0.3.0 主线交接与 0.3.1 配色修补）
...
### 当前主线状态（2026-06-16 / 0.3.1 build 29）

本地 `main` 已切到 UI 分支作为后续主线，并在此基础上继续推进 `0.3.1` 配色修补。旧主线基线 `3303606` 仅作为历史远端分支保留，不再作为后续开发入口。
...
- 发布脚本 `MeetingBotMenuBarApp/build_release_app.sh` 当前版本为 `0.3.1`、构建号 `29`；安装盘脚本版本已同步为 `0.3.1`。
...
- `dist/会议纪要助手.app`
- 默认安装盘路径：`dist/会议纪要助手 0.3.1 安装盘.dmg`
- 默认线程交接导出路径：`dist/线程交接汇总 0.3.1.md`

## 已执行验证

2026-06-16 / 0.3.1 主线执行过以下检查：
...
- 测试结果：`34 passed in 0.84s`。
- Shell 脚本语法检查通过。
- 安装前检查可执行完成；使用缺失 `.env` 路径时不会中断，当前机器仅有“无法读取物理内存”的提示级警告。
- Python 编译检查通过。
- 菜单栏 App 已重新构建，`Info.plist` 版本为 `0.3.1`，构建号为 `29`。
...
- `scripts/build_setup_package.sh` 默认版本号已同步到 `0.3.1`；本轮未重新生成 DMG。
