...
默认分发物为 `会议纪要助手 0.5.1 安装盘.dmg`。其中包含：
...
如果已经安装并运行过旧版，仍然优先使用 `会议纪要助手 0.5.1 安装盘.dmg`。把新版 App 拖入 `Applications` 覆盖旧版后，再打开 App，程序会自动识别为升级路径；如果旧版仍在 `$HOME/Library/Application Support/meetin-bot`、`$HOME/Library/Application Support/feishu-meeting-bot`、`$HOME/meetin-bot`、`$HOME/meeting-bot` 或 `$HOME/feishu-meeting-bot`，会把 `.env`、虚拟环境和用户数据迁移到新的 `$HOME/Library/Application Support/meeting-bot`，并把旧根目录下的日志迁到 `$HOME/Library/Logs/meeting-bot`。
...
- 生成默认交付物 `dist/会议纪要助手 0.5.1 安装盘.dmg`。
- 把安装与升级说明、使用说明、更新记录和开发路线图一并放入安装盘。
- 同步在 `dist/` 下生成线程交接汇总。
- 生成 `dist/release-manifest-0.5.1.json`，记录安装盘大小、SHA-256、版本和构建号；如配置 ECDSA P-256 发布密钥，同时生成签名文件。
